...
namespace DedisclasikTests
{
    ...
    public class InscriptionEmpruntTest
    {
        ...
        public void testEmprunts()
        {
            inscription.inscrire("test", "unitaire", "testdu33", "123");
            ABONNÉS abo = Outils.getAbo();
            Connexion.abonné = abo;
            List<ALBUMS> listEmprunts = EMPRUNTER.ListeAlbums();
            ...
            listEmprunts = EMPRUNTER.ListeAlbums();
            Assert.IsTrue(listEmprunts[0].TITRE_ALBUM.Trim().Equals(emp1.ALBUMS.TITRE_ALBUM.Trim()));
            Assert.IsTrue(listEmprunts[1].TITRE_ALBUM.Trim().Equals(emp2.ALBUMS.TITRE_ALBUM.Trim()));
            Assert.IsTrue(listEmprunts[2].TITRE_ALBUM.Trim().Equals(emp3.ALBUMS.TITRE_ALBUM.Trim()));
